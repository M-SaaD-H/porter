...
function getEl<T extends HTMLElement>(id: string): T {
  ...
  if (!el) throw new Error(`[PORTER] Missing DOM element: #${id}`);
  return el as T;
}
...
async function onSave(): Promise<void> {
  ...
  try {
    ...
    console.error('[PORTER] Save error:', err);
  } finally {
    btn.disabled = false;
  }
}
...
async function onFill(): Promise<void> {
  ...
  try {
    ...
    console.error('[PORTER] Fill error:', err);
  } finally {
    btn.disabled = false;
...
async function init(): Promise<void> {
  try {
    ...
    console.error('[PORTER] Init error:', err);
